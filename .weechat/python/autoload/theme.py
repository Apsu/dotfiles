...
def theme_config_init():
    ...
    theme_config_option['themes_dir'] = weechat.config_new_option(
        ...
        '%h/themes', '%h/themes', 0, '', '', '', '', '', '')
    theme_config_option['themes_backup'] = weechat.config_new_option(
        theme_config_file, section_themes,
        'backup', 'string', 'Backup of your initial theme', '', 0, 0,
        '%h/themes/backup.theme', '%h/themes/backup.theme', 0, '', '', '', '', '', '')
...
def theme_config_get_backup():
    """Return name of backup theme (by default "~/.weechat/themes/backup.theme")."""
    global theme_config_option
    return weechat.config_string(
        theme_config_option['themes_backup']).replace('%h',
                                                      weechat.info_get('weechat_dir', ''))
...
def theme_get_name_backup():
    """Return name of self theme (backuped when script is loaded)."""
    return '%s/%s' % (theme_get_dir(), THEME_BACKUP)
...
class Theme:
    ...
    def save(self, filename):
        ...
        names.sort()
        if not filename:
            filename = theme_get_self()
        try:
            f = open(filename, 'w')
            f.write('#\n# %s -- WeeChat theme written on %s [WeeChat %s / %s.py %s]\n#\n' % (
                    os.path.basename(filename), datetime.date.today(),
                    weechat.info_get('version', ''), SCRIPT_NAME, SCRIPT_VERSION))
            for option in names:
    ...
    def show(self, header):
        ...
        self.prnt(header)
        for option in names:
            self.prnt('  %s %s= %s%s' % (option, weechat.color('chat_delimiters'),
                                         weechat.color('chat_value'), self.options[option]))

    def info(self, header):
        """Display info about theme."""
        self.prnt('')
        self.prnt(header)
        self.prnt('%d options' % len(self.options))
...
class HtmlTheme(Theme):
    ...
    def to_html(self):
        ...
        nicks = ('@carl', '@jessika', '@louise', '%Melody', '%Diego', '+Max',
                 ...
                 'madeleine', self.html_nick_self, 'mila', 'peter', 'tina', 'Vince', 'warren')
        ...
        html.append(self.html_concat((('weechat.bar.input.color_delim', '['),
                                      ...
                                      ('weechat.bar.input.color_delim', ')] '),
                                      ('weechat.bar.status.color_fg',   'this is mispelled '),
...
def theme_cmd(data, buffer, args):
    ...
    if argv[0] in ('list', 'install', 'remove', 'load'):
        weechat.prnt('', '%s: action "%s" not developed' % (SCRIPT_NAME, argv[0]))
        return weechat.WEECHAT_RC_OK

    # check arguments
    if len(argv) < 2:
        if argv[0] in ('install', 'remove', 'load', 'export'):
    ...
    if import_weechat_ok:
        if weechat.register(SCRIPT_NAME, SCRIPT_AUTHOR, SCRIPT_VERSION, SCRIPT_LICENSE,
            ...
            weechat.hook_command(SCRIPT_COMMAND,
                                 'WeeChat theme manager',
                                 'list [<text>] || info|show [<theme>] || install|remove <theme> || load <file> || save [<file>]'
                                 ' || export [white] <file>',
                                 ...
                                 '    load: load theme from a file\n'
                                 '    save: save current theme in a file\n'
                                 '  export: save current theme as HTML in a file\n'
                                 '   white: use white background in HTML file\n\n'
                                 'Examples:\n'
